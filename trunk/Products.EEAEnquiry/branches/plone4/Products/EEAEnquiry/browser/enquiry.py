...
class SendEnquiry(BrowserView):
    ...
    def sendUnsentEnquiries(self):
        ...
        return self.request.response.redirect( context.absolute_url() + '/enquiry_sent_confirmation')
            ...
            if subscriber.subscribed:
                if nt_ob.alreadySubscriber(subscriber_email):
                    ...
                    if nt_subscriber is not None:
                        nt_subscriber.edit(format='HTML', active=subscriber.subscribed, email=subscriber_email)
                else:
...
class SubmitEnquiry(BrowserView):
    ...
    def getErrors(self):
        ...
        if self.request.get('login', None) is not None:
            ...
            if password == '':
                errors['password'] = "Please enter your password, if you have forgotten your password press 'please send my password'"
    ...
    def enquiryStep2(self):
        ...
        if len(requestors) == 0:
            ...
            if reqObj is None or reqObj.portal_type != 'EnquiryRequestor':
                reqId = folder.invokeFactory(type_name='EnquiryRequestor', id=enquiry, title='' )
        ...
        if self.request.get('login', None) is not None:
            for req in requestors:
                ...
                if obj.getPassword() == password:
                    ...
                    return self.request.response.redirect(obj.absolute_url() + '/edit?enquiry=%s' % enquiry )
        ...
        if self.request.get('mailpassword', None) is not None:
            ...
            if len(requestors)>0:
                ...
                mail_text = context.enquiry_mail_password_template( context
                                                                 , self.request
                                                                 , email=email
                                                                 , password=password
                ...
                host.send( mail_text )
                return self.request.response.redirect( context.absolute_url() + '/mail_password_response')
...
class EnquiryRequestor(BrowserView):
    ...
    def getUserInfo(self, userid):
        ...
        cat = getToolByName(context, 'portal_enquiry_catalog')
        requestors = cat.searchResults( portal_type = 'EnquiryRequestor', Title = userid )
        if len(requestors) > 0:
